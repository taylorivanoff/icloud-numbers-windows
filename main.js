...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({ width: 1920, height: 1080, webPreferences: { nodeIntegration: true, partition: 'persist:icloud-numbers' } });
  ...
  mainWindow.loadURL('https://www.icloud.com/numbers');
...
function createTray() {
  ...
  tray = new Tray(icon);
  tray.setToolTip('iCloud Numbers');
  tray.setContextMenu(Menu.buildFromTemplate([
    { label: 'Show iCloud Numbers', click: () => { if (!mainWindow) createWindow(); mainWindow.show(); } },
...
app.on('before-quit', async () => { await session.fromPartition('persist:icloud-numbers').cookies.flushStore(); });
